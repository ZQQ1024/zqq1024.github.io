...
所有大于 3 的质数在模 6 下只能是 ±1 的余数，即只能写成 6n+1 或 6n−1，但并非所有 6n±1 都必然是质数

证明：
把一个数 p（大于 3）放到模 6 的环境中考察，一个整数除以 6 的余数只可能是

0, 1, 2, 3, 4, 5. 仅余数 1 或 5（即 ±1(mod6)）是可能的质数，其余能被 2 或 3 整除。得证

```python
def is_prime(n: int) -> bool:
    # 处理小于等于3的情况：2和3是质数；1或更小则不是
    if n <= 3:
        return n > 1
    ...
    # 说明输入的数，没有2 3 因子，如果他不是质数，那么对他做质因数分解，肯定存在大于等于5的质因子
    # 并不是所有6n +- 1 的数都是质因数，但是不影响代码，执行，因为找到其他因子也行
    # 从5开始，每次步进6，检查 i 和 i+2 这两个位置
    i = 5
    while i * i <= n:
        if n % i == 0 or n % (i + 2) == 0:
            return False
...
因为2x3=6，以6为步长的数中，能排除2的倍数 3的倍数；同理完全可以用 2×3×5 = 30 做步长，但是那样代码会较为复杂，选取6是执行效率以及代码编写效率的综合考虑。

真正需要针对大整数做判定时，往往会使用更先进的算法(Miller-Rabin、AKS 等)
{{< /hint >}}
