...
4. 在模 的意义下，方程组 (S) 只有一个解：x = a1t1M1 + a2t2M2 + ... + antnMn
...
2. e 的取值
{{< expand "" "..." >}}
> e 的取值是安全性和性能的综合考虑，e 如果具有较短的bit长度和较小的汉明重量则会提升算法性能。更常见 e 的取值为 2^16 + 1 = 65537（对应十六进制为 0x10001），最小的取值为 3，性能也最好。一般实现如 PKCS#1 是先固定 e 的值，如固定 e 为 65537，然后确定 φ(n) 不是 e 的倍数即可
{{< /expand >}}
...
{{< expand "" "..." >}}
> 根据素数定理，在 k 位十进制正整数中遇到素数的概率约等于 2/ln(10^k)，这个概率并不低；同时可以借助素数检测类方法如米勒检测法，可以快速地判断一个数是不是素数。综合起来，得到两个大素数并不需要花很多时间以及计算机算力
{{< /expand >}}
